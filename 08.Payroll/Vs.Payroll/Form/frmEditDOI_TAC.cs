...
namespace Vs.Payroll
{
    public partial class frmEditDOI_TAC : DevExpress.XtraEditors.XtraForm
    {
                    ...
                    case "luu":
                        {
                            ...
                            Commons.Modules.sId = Convert.ToString(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, "spUpdateDOI_TAC", (AddEdit ? -1 : Id), txtMA_SO.EditValue.ToString(), txtTEN_NGAN.EditValue.ToString(), txtTEN_CTY_DAY_DU.EditValue.ToString(), chkINACTIVE.EditValue));
                            if (AddEdit)
                            {
        ...
        private void LoadText()
        {
            try
            {
                string sSql = "SELECT ID_DT, MA_SO, TEN_NGAN, TEN_CTY_DAY_DU, INACTIVE " +
