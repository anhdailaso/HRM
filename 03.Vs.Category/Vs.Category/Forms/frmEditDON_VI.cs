...
namespace Vs.Category
{
    public partial class frmEditDON_VI : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void LoadText()
        {
            try
            {
                string sSql = "SELECT ID_DV ,MSDV ,TEN_DV ,TEN_DV_A ,TEN_DV_H ,TEN_NGAN ,DIA_CHI ,MAC_DINH ,CHU_QUAN ,DIEN_THOAI ,FAX ,MS_BHYT ,MS_BHXH ,SO_TAI_KHOAN ,TEN_NGAN_HANG ,KY_HIEU ,NGUOI_DAI_DIEN ,CHUC_VU ,SO_HS,STT_DV,PC_CN FROM dbo.DON_VI WHERE ID_DV =	" + iIdDV.ToString();
                ...
                ItemForPC_CN.Control.Text = dtTmp.Rows[0]["PC_CN"].ToString();
               if (bAddEditDV)
                {
                    ...
                    case "luu":
                        {
                            ...
                            SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, "spUpdateDonVi", (bAddEditDV ? -1 : iIdDV), ItemForMSDV.Control.Text, 
                                    ...
                                    ItemForSO_HS.Control.Text, ItemForSTT_DV.Control.Text, ItemForPC_CN.Control.Text).ToString();
